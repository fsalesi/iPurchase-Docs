...
1. **Hardcoded LIST:** `LIST:code:description,code:description`
   ```
   CODE_LIST_REJECTION_CODES = LIST:001:Invalid Prices,002:No Budget,003:Other
   ```

2. **QAD Generalized Code:** Point to a code_mstr field name
   ```
...
3. **Blank (some settings):** Pulls from QAD master data
   - `CODE_LIST_H_SHIPTO` → si_mstr
   - `CODE_LIST_H_CURRENCY` → cu_mstr
   - `CODE_LIST_UM` → um_mstr
